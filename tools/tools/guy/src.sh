...
package()
{
  ...
  for target_arch in ${target_archs}; do
    ...
    env DISTDIR="${workdir}" make distributeworld
    cd tools/tools/guy
    env DESTDIR="${workdir}"/base make -m $(realpath ../../../share/mk) delete-optional
    ...
    for kern in ${kernconf}; do
      env DISTDIR="${workdir}"/base make distributekernel KERNCONF="${kern}"
